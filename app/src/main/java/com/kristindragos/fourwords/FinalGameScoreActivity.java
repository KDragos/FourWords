...
public class FinalGameScoreActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!extrasBundle.isEmpty()) {
            ...
            if(hasLengthOfBestWords) {
                int lengthOfBestWords = (int) extrasBundle.get("lengthOfBestWords");
            }
            if(hasNumNonWords) {
    ...
    private void updateUI() {
        ...
        ArrayAdapter<String> playerAdapter = new ArrayAdapter<>(getApplicationContext(), R.layout.previous_words, R.id.previous_words_textView, playerWords);
        ...
        if (bestWords.size() == 1) {
            words += bestWords.get(0);
        } else {
            for (int i = 0; i < this.bestWords.size(); i++) {
                words += bestWords.get(i);
